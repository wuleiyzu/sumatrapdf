...
// we accept args -foo and /foo
static const WCHAR* SkipArgSpecifier(const WCHAR* s) {
    if (*s == L'-') {
        return s + 1;
    }
    if (*s == L'/') {
...
static bool IsArgEq(const WCHAR* s, const WCHAR* s2) {
    s = SkipArgSpecifier(s);
    s2 = SkipArgSpecifier(s2);
    if (!s || !s2) {
        return false;
    }
    return str::Eq(s, s2);
...
static WCHAR* EatParam(WStrVec& args, size_t& i) {
    size_t n = args.size();
    if (n >= i) {
        return nullptr;
    }
...
static WCHAR* EatParamDup(WStrVec& args, size_t& i) {
    size_t n = args.size();
    if (n >= i) {
        return nullptr;
    }
...
static WCHAR* GetParam(WStrVec& args, size_t& i) {
    size_t n = args.size();
    if (n >= i) {
        return nullptr;
    }
...
#define has_additional_param() ((nArgs > n + 1) && ('-' != args.at(n + 1)[0]))
#define handle_int_param(name) name = _wtoi(args.at(++n))

#define isArg(name) IsArgEq(arg, name)

    WCHAR* arg;
    WCHAR* param;
    // starting at 1 because 0 is exe path
    for (size_t n = 1; n < nArgs; n++) {
        arg = args.at(n);
        int nParams = nArgs - n - 1;
        ...
        if (isArg(L"-x")) {
            ...
            // (if /d isn't used)
            i.silent = true;
            // TODO: this must be done after paring all flags
            if (!i.installDir) {
        ...
        if (isArg(L"-esc-to-exit")) {
            i.globalPrefArgs.Append(str::Dup(arg));
            continue;
        }
        ...
        // the rest of arguments require at least one param
        if (nParams == 0) {
        ...
        param = args.at(n + 1);
        if (isArg(L"-print-to")) {
            i.printerName = EatParamDup(args, n);
            i.exitWhenDone = true;
            continue;
        }
        if (isArg(L"-print-settings")) {
            ...
            i.printSettings = EatParamDup(args, n);
        ...
        if (isArg(L"-n")) {
           i.stressParallelCount = _wtoi(param);
            n++;
            continue;
        }
        if (isArg(L"-render")) {
        ...
        if ((isArg(L"-forward-search") || isArg(L"-fwdsearch")) && (nParams >= 2)) {
            ...
            n++;
            param = args.at(n);
            i.forwardSearchLine = _wtoi(param);
            n++;
            continue;
        }
        if (isArg(L"-plugin")) {
            ...
            if (nParams >= 2 && !str::IsDigit(*param) && (*args.at(n+2) != '-')) {
        ...
        if (isArg(L"-stress-test")) {
            ...
            if (has_additional_param() && str::FindChar(additional_param(), '*')) {
                i.stressTestFilter = EatParamDup(args, n);
            }
            if (has_additional_param() && IsValidPageRange(additional_param())) {
               i.stressTestRanges = EatParamDup(args, n);
            }
        ...
        if (isArg(L"-set-color-range") && nArgs > n + 2) {
            i.globalPrefArgs.Append(str::Dup(args.at(n)));
            i.globalPrefArgs.Append(str::Dup(args.at(++n)));
            i.globalPrefArgs.Append(str::Dup(args.at(++n)));
            continue;
    ...
    ParseAsFile : {
        ...
        if (str::EndsWithI(arg, L".lnk")) {
            filePath = ResolveLnk(arg);
        }
        if (!filePath) {
            filePath = str::Dup(arg);
        }
